...
def build_parser():
    ...
    init_parser.add_argument("--log", metavar='PUBKEY', type=bytes.fromhex, help="Select a specific log from the policy file")
    ...
    poll_parser.add_argument("--log", metavar='PUBKEY', type=bytes.fromhex, help="Select a specific log from the policy file")
...
def do_init(args: argparse.Namespace):
    with open(args.state_dir / 'policy', 'r') as f:
        log = SigsumLogAPI.from_policy(f.read(), log_key=args.log)
